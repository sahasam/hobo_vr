"""
pyvr track-dev
experimental tracker to handle my own necessary implementations

Usage:
    pyvr track-dev [options]
...
class Poser(templates.PoserTemplate):
    ...
    def __init__(self, *args, camera=4, width=-1, height=-1, calibration_file=None, **kwargs):
        ...
        self.serialPaths = {"green": "/dev/ttyUSB1", "blue": "COM3"}
                    ...
                    with serial.threaded.ReaderThread(ser2, u.SerialReaderFactory) as protocol:
                        ...
                        while self.coro_keep_alive["serial_listener"][0]:
                            try:
                                ...
                                if len(quat) > 0:
                                    self.pose.r_w = round(quat[0], 4)
                                    self.pose.r_x = round(quat[1], 4)
                                    self.pose.r_y = round(quat[2], 4)
                                    self.pose.r_z = round(quat[3], 4)

                                    if self._serialResetYaw:
                                        yaw_offset = 0 - ypr[0]

                                    print(self.pose.r_w, self.pose.r_x, self.pose.r_y, self.pose.r_z)
